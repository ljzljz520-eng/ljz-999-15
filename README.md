...
*   **维修历史关联**: 查询设备时同步展示保修状态、最近维修单、换件记录与未关闭问题。
*   **领用状态管控**: 维修中（含存在未关闭维修单）的设备明确标识"不可领用"，防止误领。
*   **分离式导出**: 资产基础信息与维修备注导出为两个独立 CSV 文件，查阅不混淆。
...
*   **固定资产编码**: `FA001`
*   **预期结果**: `SN2024001`，保修状态"在保"，标识"可正常领用"
...
| `FA002` | 维修中，有未关闭问题与换件记录 | **不可领用（设备维修中）** |
| `FA003` | 已过保，历史维修已关闭 | 可正常领用，保修状态"已过保" |
| `FA004` | 使用中（已领用） | 不可领用（设备已被领用） |
| `FA005` | 台账闲置但挂着未关闭维修单 | 不可领用（存在未关闭维修单） |
| `TEST-01` | 无扩展信息（模拟旧数据） | 仅显示 SN（降级模式） |
